...
exports.commitItem = functions.https.onCall(async (data, context) => {
    ...
    await (0, accessControlService_1.verifyMemberAccess)(collectionId, actorUserId, ['owner', 'manager', 'editor']);
    ...
    try {
        ...
        if (result.success) {
            ...
            await (0, auditService_1.logAuditEvent)(collectionId, actorUserId, isDelete ? 'item_deleted' : 'item_committed', isDelete ? `Deleted item ${itemId}` : `Committed item ${itemId}`, { item_id: itemId });
        }
        return result;
    }
    catch (err) {
