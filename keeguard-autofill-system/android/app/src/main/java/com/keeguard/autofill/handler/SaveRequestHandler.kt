package com.keeguard.autofill.handler
...
class SaveRequestHandler(
    ...
    // Launches the in-app save bottom sheet via a broadcast to the React Native layer
    fun launchSaveUI(result: SaveResult, parsedForm: ParsedForm) {
        when (result) {
            is SaveResult.NeedsNewSave -> {
                val intent = Intent("com.keeguard.AUTOFILL_SAVE").apply {
                    putExtra("action", "new")
