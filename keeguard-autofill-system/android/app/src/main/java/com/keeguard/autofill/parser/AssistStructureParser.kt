 ...
 * approach — original implementation written for KeeGuard.
 */
class AssistStructureParser {
    ...
    private fun classifyNode(node: ViewNode): FieldType {
        ...
        if (hints.any { it.contains("newPassword", true) || it.contains(View.AUTOFILL_HINT_NEW_PASSWORD, true) }) return FieldType.NEW_PASSWORD
